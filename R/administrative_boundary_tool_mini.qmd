...
Districts are based on pre-2024 parliamentary constituencies.
...
Localities are built up from districts (pre-2024 parliamentary constituencies), with two districts making up each locality.
...
These maps visualise the interaction between wards and 2024 parliamentary constituencies in Birmingham. Some wards fit neatly inside a constituency boundary; others do not. Hovering over a ward will show you what proportion of that ward fits inside the constituency boundary.
...
This section demonstrates the potential ways a district/locality model could be achieved.

- **Model A:** the existing model. Districts are based on pre-2024 constituency boundaries. Wards and districts are not always coterminous.
- **Model B:** a constituency-based model. Districts are based on post-2024 constituency boundaries. Wards and districts are not always coterminous.
- **Model C:** a ward-up model using the Health and Wellbeing Board proposal (29/1/26). Wards and districts are always coterminous but Selly Oak is not contiguous.
- **Model D:** a ward-up model using best fit alignment with existing district boundaries (pre-2024 constituencies). Wards are assigned to the district they share the largest footprint with. Wards and districts are always coterminous.
...
The map tab shows the four potential district models described above. Use the menu in the top right to move between models. Ward boundaries are shown in light grey.

The other tabs provide a lookup between wards and districts for each model.
...
## Model C
```{r district-model-c-table}
ward_district_locality_lookup |> 
...
The map tab shows the four potential locality models, based on the district models described above. Two districts make up each locality. Use the menu in the top right to move between models. Ward boundaries are shown in light grey.

The other tabs provide a lookup between wards and localities for each model.
...
## Model B
```{r locality-model-b-table}

# calculate % of wards in each proposed locality
locality_pcon24_wards_full <- data.frame()
locality_pcon24_wards <- data.frame()

sf_use_s2(FALSE)

localities <- proposed_bham_localities_pcon24$locality

for (loc in localities) {
  # clip wards inside district boundary
  temp_locality_pcon24_wards <- proposed_bham_localities_pcon24 |> 
    filter(locality == loc)|> 
    st_intersection(bham_ward25_boundaries) |> 
    mutate(locality = loc)
  
  # find out the area of each clipped ward
  temp_locality_pcon24_wards$area <- st_area(temp_locality_pcon24_wards)
  
  # pull out unclipped wards for this locality
  temp_locality_pcon24_wards_full <- bham_ward25_boundaries |> 
    filter(wd25cd %in% temp_locality_pcon24_wards$wd25cd) |> 
    mutate(locality = loc)
  
  # find area of unclipped ward and add to clipped ward sf
  temp_locality_pcon24_wards$full_area <- st_area(temp_locality_pcon24_wards_full)
  
  # calulate what % of each ward is inside the boundary
  # then filter out wards with less than 1% area inside boundary
  temp_locality_pcon24_wards <- temp_locality_pcon24_wards |> 
    mutate(pct = area/full_area*100) |> 
    filter(as.numeric(pct) > 1)
  ...
  # append temp dfs to main df
  locality_pcon24_wards_full <- rbind(locality_pcon24_wards_full,
                               temp_locality_pcon24_wards_full)
  
  locality_pcon24_wards <- rbind(locality_pcon24_wards,
                          temp_locality_pcon24_wards)
}

locality_pcon24_wards |> 
  as_tibble() |> 
  select(wd25nm, locality, pct) |> 
  arrange(locality) |> 
  mutate(pct = round(pct, 0),
