...
## Locality-District Interaction Maps

These maps visualise the interaction between districts and localities in Birmingham. This is a rare case where all districts are coterminous with locality boundaries. There are two districts in each locality.

Use the tabs to move between localities. A table is provided in the final tab as a quick lookup between localities and districts - this can be filtered or sorted as needed.

```{r locality-district-map-setup}
locality_districts_full <- data.frame()
locality_districts <- data.frame()
...
for (loc in localities) {
  ...
  temp_locality_districts <- bham_locality_boundaries |> 
    filter(locality == loc)|> 
    st_intersection(bham_district22_boundaries) |> 
    mutate(locality = loc)
  
  # find out the area of each clipped pcon
  temp_locality_districts$area <- st_area(temp_locality_districts)
  
  # pull out unclipped pcons for this locality
  temp_locality_districts_full <- bham_district22_boundaries |> 
    filter(pcon22nm %in% temp_locality_districts$pcon22nm) |> 
    mutate(locality = loc)
  
  # find area of unclipped pcon and add to clipped pcon sf
  temp_locality_districts$full_area <- st_area(temp_locality_districts_full)
  
  # calulate what % of each pcon is inside the boundary
  # then filter out pcons with less than 1% area inside boundary
  temp_locality_districts <- temp_locality_districts |> 
    mutate(pct = area/full_area*100) |> 
    filter(as.numeric(pct) > 1)
  ...
  # append temp dfs to main df
  locality_districts_full <- rbind(locality_districts_full,
                               temp_locality_districts_full)
  
  locality_districts <- rbind(locality_districts,
                          temp_locality_districts)
}

# function for making locality maps
make_locality_district_map <- function(loc) {
  # wrapped pcon names for district map
  label_district_wrap_locality <- locality_districts |> 
    filter(locality == loc) |> 
    pull(pcon22nm) |> 
    str_wrap(15)
  label_district_wrap_locality<- gsub("\n", "<br>", label_district_wrap_locality) |> 
    lapply(htmltools::HTML)
  
  # labels with % inside locality for locality map
  label_district_locality <- locality_districts |> 
    filter(locality == loc) |> 
    mutate(label = paste0(pcon22nm, " district", "<br>",
                          round(pct, 0), "% falls within ", locality, " locality")) |> 
    pull(label) |> 
    lapply(htmltools::HTML)
  ...
  leaflet(width = "100%", height = "650px") |> 
    ...
    addPolygons(data = bham_locality_boundaries |> filter(locality == loc),
                ...
                fillColor = "white",
                options = pathOptions(pane = "localityPane")) |>
    # display IMD by LSOA for locality of interest only
    addPolygons(data = locality_lsoas |> filter(locality == loc),
                fillColor = ~imd25_pal(imd25_decile),
                fillOpacity = 0.7,
                color = ~imd25_pal(imd25_decile),
    ...
    addPolygons(data = locality_districts_full |> filter(locality == loc),
                color = border_colour,
                weight = 0,
                opacity = 0,
                fillOpacity = 0,
                fillColor = "white",
                label = label_district_wrap_locality,
                labelOptions = labelOptions(noHide = T, textOnly = T, opacity = 1,
...
```{r north-district-map}
make_locality_district_map("North")
```

## East

```{r East-district-map}
make_locality_district_map("East")
```

## South

```{r South-district-map}
make_locality_district_map("South")
```

## West

```{r West-district-map}
make_locality_district_map("West")
```

## Central

```{r Central-district-map}
make_locality_district_map("Central")
```

## Table

```{r locality-district-table}
locality_districts |> 
  as_tibble() |> 
  select(pcon22nm, locality, pct) |> 
  arrange(pcon22nm) |> 
  mutate(pct = round(pct, 0),
         pct = paste0(pct, "%")) |> 
  rename(`District name` = pcon22nm,
         `Locality name` = locality,
         `% of district in locality` = pct) |> 
  datatable(rownames = F,
            filter = "top",
            options = list(pageLength = 15,
                           autoWidth = TRUE))
...
## Locality-Constituency Interaction Maps
